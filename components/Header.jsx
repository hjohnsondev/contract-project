                    ...
                    <div className="flex justify-between items-center h-full">
                        <div className="flex space-x-8 h-full">
                            {navigationItems?.map((item, index) => {
                                return (
                                    <div key={index} className="flex items-center h-full group">
                                        <Link href={`/${item?.fields?.label != "Home" ? item?.fields?.label.toLowerCase() : ""}`}><button id={`dropdown${item?.fields?.label}Button`} data-dropdown-toggle={`dropdown${item?.fields?.label}`} className={`py-4 ${router.pathname.includes(item.fields?.label.toLowerCase()) || router.pathname == "/" && item?.fields?.label == "Home" ? "text-sky-500" : "text-white"} group-hover:text-sky-500`}><strong>{item?.fields?.label}</strong></button></Link>
                                        <div className="ml-0 group-hover:rotate-180">{navArrow}</div>
                                    </div>
                                )
                            })}
